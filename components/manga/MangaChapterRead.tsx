...
import { fetchMangaChapterImages } from "@/actions/GetApiData";
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col gap-4 items-center p-4">
        ...
        <p className="text-muted-foreground text-sm">Loading chapter pages...</p>
      </div>
    );
  }
  ...
  if (isError || !images || images.length === 0) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <p className="text-destructive text-lg">Failed to load chapter images.</p>
      </div>
    );
  }
