...
const MangaCardWrapper = ({ data }: Props) => {
  ...
  return (
    <Link href={`/manga/${id}`} className="group block">
      <Card className="relative h-[340px] overflow-hidden rounded-xl border border-slate-700 bg-slate-950 text-white shadow-xl transition-transform duration-200 group-hover:scale-[1.02]">
        ...
        <CardContent className="absolute bottom-0 z-10 w-full bg-gradient-to-t from-black/80 via-black/50 to-transparent p-4 backdrop-blur-md space-y-2">
          ...
          <div className="flex gap-2 font-bold">
            {attributes.status && (
              <Badge className="text-xs font-medium px-1  bg-emerald-600/80 backdrop-blur-sm text-white shadow-md">
                {attributes.status}
              </Badge>
            )}
            {attributes.contentRating && (
              <Badge className="text-xs font-medium  bg-red-600/90 text-white shadow-md">
                {attributes.contentRating}
              </Badge>
            )}
          </div>
          ...
          {attributes.tags?.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {attributes.tags.slice(0, 4).map((tag, i) => (
                <Badge
                  key={i}
